...
const { data: crawls } = await useAsyncData(
  'dashboard-crawls-upcoming',
  async () => {
...
const filteredCrawls = computed(() => {
  const list = crawls.value ?? [];

  const upcomingWithTheme = list
    .filter((crawl) => crawl.event_date_start && crawl.theme?.id)
    .sort((a, b) => {
      const aDate = new Date(a.event_date_start as string).getTime();
      const bDate = new Date(b.event_date_start as string).getTime();
      return aDate - bDate;
    });

  const nextThemeId = upcomingWithTheme[0]?.theme?.id ?? null;

  if (!nextThemeId) {
    return upcomingWithTheme.length ? upcomingWithTheme : list;
  }

  return list.filter((crawl) => crawl.theme?.id === nextThemeId);
});

const tableData = computed<CrawlRow[]>(() =>
  filteredCrawls.value.map((crawl) => ({
    id: crawl.id,
